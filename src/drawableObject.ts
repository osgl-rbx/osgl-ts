...
import { BaseUnloadedTexture, Drawable, DrawableObject, OSGLBitmap, OSGLWindow, RawTexture } from "./types"

export declare const Window: {
    new: (editableImage: EditableImage, renderers: Drawable[]) => OSGLWindow,
    from: (drawableObject: Drawable, width: number, height: number) => Result<OSGLWindow, WindowError>,
    fromAssetId: (assetId: string) => Result<OSGLWindow, WindowError>,
    fromBuffer: (buffer: buffer, width: number, height: number) => Result<OSGLWindow, WindowError>,
...
export declare const Texture: {
    ...
    serializeChecked: (rawTexture: RawTexture) => Result<[buffer, number, number], TextureError>,
    serializeUnchecked: (rawTexture: BaseUnloadedTexture) => [buffer, number, number],
    drawChecked:<T>(object: DrawableObject<T>, texture: Texture, textureX: number, textureY: number, rotation?: number) => null,
    drawUnchecked:<T>(object: DrawableObject<T>, texture: Texture, textureX: number, textureY: number, rotation?: number) => null,
}
